...
<div class="w-full">
    ...
    <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-5">
        ...
        <div class="grid sm:grid-cols-2 gap-5">
            ...
            <div>
                ...
                <select name="variant" required class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-miruku-blue">
                    ...
                    <option value="{{ $v->slug }}" {{ old('variant', $product->variant) === $v->slug ? 'selected' : '' }}>{{ $v->name }} ({{ $v->icon }})</option>
                    @endforeach
                </select>
            </div>
            <div>
            ...
            <div>
                ...
                <select name="unit" required class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-miruku-blue">
                    @foreach($units as $u)
                    <option value="{{ $u->slug }}" {{ old('unit', $product->unit) === $u->slug ? 'selected' : '' }}>{{ $u->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
        ...
        <div class="flex flex-wrap items-center gap-x-8 gap-y-4">
            ...
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="show_on_home" {{ old('show_on_home', $product->show_on_home) ? 'checked' : '' }} class="rounded border-gray-300 text-miruku-blue">
                <span class="text-sm font-medium text-gray-700">Tampilan di Halaman Utama</span>
            </label>
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="is_best_seller" {{ old('is_best_seller', $product->is_best_seller) ? 'checked' : '' }} class="rounded border-gray-300 text-miruku-blue">
...
<script>
    ...
    @if(old('benefits'))
        ...
        @endforeach
    @elseif($product->benefits && count($product->benefits) > 0)
        @foreach($product->benefits as $index => $idVal)
            addBenefit('{{ $idVal }}', '{{ $product->benefits_en[$index] ?? '' }}');
        @endforeach
    @else
        addBenefit();
    @endif
