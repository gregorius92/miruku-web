...
<div class="w-full">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-900">Tambah Varian</h1>
    </div>

    <form action="{{ route('admin.variants.store') }}" method="POST" class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-5">
        @csrf
        <div class="grid sm:grid-cols-2 gap-5">
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">Nama Varian (ID) *</label>
                <input type="text" name="name" id="name_id" value="{{ old('name') }}" required class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-miruku-blue focus:ring-2 focus:ring-blue-50" placeholder="Contoh: Cokelat">
            </div>
            <div>
                ...
                <input type="text" name="name_en" id="name_en" value="{{ old('name_en') }}" class="w-full border border-blue-100 bg-blue-50/20 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-miruku-blue" placeholder="Example: Chocolate">
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">Slug / Key *</label>
                <input type="text" name="slug" id="slug" value="{{ old('slug') }}" required class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-miruku-blue" placeholder="example-slug">
                <p class="text-[10px] text-gray-400 mt-1">Slug digunakan sebagai kunci teknis (misal: 'original', 'chocolate').</p>
            </div>
            <div>
                ...
                <select name="icon" required class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-miruku-blue">
                    @foreach(['🥛', '🍫', '🍌', '🍓', '🍵', '☕', '🍨', '🍯', '🥭', '🫐', '🥥', '🍈'] as $emoji)
                        <option value="{{ $emoji }}" {{ old('icon') == $emoji ? 'selected' : '' }}>{{ $emoji }}</option>
                    @endforeach
                </select>
            </div>
            <div class="sm:col-span-2">
                ...
                <select name="color_class" class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-miruku-blue">
                    @foreach([
                        ...
                        <option value="{{ $class }}" {{ old('color_class') == $class ? 'selected' : '' }}>{{ $label }} ({{ $class }})</option>
                    @endforeach
                </select>
        ...
        <div class="flex justify-end items-center gap-3 pt-6 border-t border-gray-100 mt-8">
            ...
            <button type="submit" class="bg-miruku-blue hover:bg-miruku-dark text-white font-semibold px-6 py-2.5 rounded-xl transition-colors text-sm">Simpan Varian</button>
        </div>
    </form>
</div>
<script>
