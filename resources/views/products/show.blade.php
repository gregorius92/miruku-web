...
<div class="pt-24 pb-16 bg-white">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-8">
        <nav class="flex items-center gap-2 text-sm text-gray-400">
            <a href="{{ route('home') }}" class="hover:text-miruku-blue transition-colors">Beranda</a>
            <span>/</span>
            <a href="{{ route('products.index') }}" class="hover:text-miruku-blue transition-colors">Produk</a>
            <span>/</span>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid lg:grid-cols-2 gap-16 items-start mb-20">
            ...
            <div class="pt-4">
                <span class="inline-flex items-center gap-1 text-xs font-bold uppercase tracking-wider text-miruku-blue bg-blue-50 px-3 py-1.5 rounded-full mb-4">
                    Varian {{ ucfirst($product->variant) }}
                </span>
                ...
                <div class="flex items-center gap-3 mb-6">
                    ...
                    <span class="text-gray-600 text-sm font-medium">{{ number_format($product->average_rating, 1) }} / 5 · {{ $reviews->count() }} ulasan</span>
                </div>
                @endif
                ...
                <div class="flex items-center gap-2 mb-8">
                    ...
                    <span class="text-green-600 text-sm font-medium">Tersedia ({{ $product->stock }} unit)</span>
                    @else
                ...
                <div class="bg-blue-50 rounded-2xl p-5 mb-8">
                    <h4 class="font-semibold text-gray-900 mb-3 text-sm uppercase tracking-wide">Yang ada di dalam Miruku</h4>
                    <div class="grid grid-cols-2 gap-2">
                        @foreach(['0% Lactose', 'Kaya Kalsium', 'Tanpa Pengawet', 'Mudah Dicerna', 'Protein Tinggi', 'Vitamin D'] as $benefit)
        ...
        <div class="border-t border-gray-100 pt-16 mb-16">
            <h2 class="text-3xl font-bold text-gray-900 mb-10 font-cormorant">Ulasan Pelanggan</h2>
            @if($reviews->count() > 0)
            ...
            @else
            <p class="text-gray-400 mb-10">Belum ada ulasan untuk produk ini. Jadilah yang pertama!</p>
            @endif

            <!-- Submit Review -->
            <div class="max-w-xl bg-blue-50 rounded-2xl p-6 border border-blue-100">
                <h3 class="font-bold text-gray-900 mb-4">Tulis Ulasanmu</h3>
                <form action="{{ route('reviews.store') }}" method="POST" class="space-y-4">
                    @csrf
                    <input type="hidden" name="product_id" value="{{ $product->id }}">
                    <input type="text" name="name" placeholder="Namamu" required class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-miruku-blue">
                    ...
                    <textarea name="comment" rows="3" placeholder="Tulis ulasanmu..." required class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-miruku-blue resize-none"></textarea>
                    <button type="submit" class="w-full bg-miruku-blue hover:bg-miruku-dark text-white font-semibold py-3 rounded-xl transition-colors">Kirim Ulasan</button>
                </form>
            </div>
        </div>
        ...
        @if($related->count() > 0)
        <div>
            <h2 class="text-3xl font-bold text-gray-900 mb-8 font-cormorant">Produk Lainnya</h2>
